...
Promises sollen nun eingesetzt werden, um das Anti-Pattern _Pyramid of Doom_ oder auch _Callback Hell_ zu vermeiden. Doch wie können wir das durch Promises erreichen.

Stellen wir uns vor, wir haben zwei asynchrone Methodenaufrufe und möchten nach dessen Fertigstellung eine Logik anstoßen (die Ladeanimation verstecken).
...
Durch hier setzt die Promise-Strategie auf uns gibt uns die Möglichkeit folgenden Syntaxzucker zu verwenden:
...
Doch meisten sollen solche asynchronen Aufrufe ein Ergebnis zurückliefern. Das Ergebnis wird als Parameter in das `resolve` übergeben.
...
~~~ javascript
function firstAsyncCall(someValue) {
...
Ich werde für dieses Beispiel genau diese Methode drei Mal verwenden. Sie verzögert die Rückgabe, inkrementiert den übergeben Wert und liefert das Ergebnis zurück.

~~~ javascript
firstAsyncCall(0)   //0++
.then(firstAsyncCall) //1++
.then(firstAsyncCall)  // 2++
.then(fistAsyncCall)  //3++
.then(function(value){
   console.log(value);  // 4
});
~~~
